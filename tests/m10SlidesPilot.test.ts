...
const pilotLessonIds = ['aula-10-1', 'aula-10-2', 'aula-10-3']
...
for (const aulaId of pilotLessonIds) {
  ...
  assert.equal(slides.length, 5, `${aulaId} deve ter 5 slides no piloto M10`)
  ...
  slides.forEach((slide, index) => {
    ...
    assert.ok(
      slide.imagem_url?.startsWith(`/slides/m10/${aulaId}-`),
      `${slide.titulo} precisa apontar para um infografico do M10`,
    )
