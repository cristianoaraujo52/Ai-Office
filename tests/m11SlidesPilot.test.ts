...
const pilotLessonIds = ['aula-11-1', 'aula-11-2', 'aula-11-3', 'aula-11-4', 'aula-11-5']
...
for (const aulaId of pilotLessonIds) {
  ...
  assert.equal(slides.length, 5, `${aulaId} deve ter 5 slides no piloto M11`)
  ...
  slides.forEach((slide, index) => {
    ...
    assert.ok(
      slide.imagem_url?.startsWith(`/slides/m11/${aulaId}-`),
      `${slide.titulo} precisa apontar para um infografico do M11`,
    )
