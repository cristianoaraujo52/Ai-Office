...
const pilotLessonIds = ['aula-4-1', 'aula-4-2', 'aula-4-3']
...
for (const aulaId of pilotLessonIds) {
  ...
  assert.equal(slides.length, 5, `${aulaId} deve ter 5 slides no piloto M04`)
  ...
  slides.forEach((slide, index) => {
    ...
    assert.ok(
      slide.imagem_url?.startsWith(`/slides/m04/${aulaId}-`),
      `${slide.titulo} precisa apontar para um infografico do M04`,
    )
