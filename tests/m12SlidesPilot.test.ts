...
const pilotLessonIds = ['aula-12-1', 'aula-12-2', 'aula-12-3']
...
for (const aulaId of pilotLessonIds) {
  ...
  assert.equal(slides.length, 5, `${aulaId} deve ter 5 slides no piloto M12`)
  ...
  slides.forEach((slide, index) => {
    ...
    assert.ok(
      slide.imagem_url?.startsWith(`/slides/m12/${aulaId}-`),
      `${slide.titulo} precisa apontar para um infografico do M12`,
    )
