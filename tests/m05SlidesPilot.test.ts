...
const pilotLessonIds = ['aula-5-1', 'aula-5-2', 'aula-5-3', 'aula-5-4']
...
for (const aulaId of pilotLessonIds) {
  ...
  assert.equal(slides.length, 5, `${aulaId} deve ter 5 slides no piloto M05`)
  ...
  slides.forEach((slide, index) => {
    ...
    assert.ok(
      slide.imagem_url?.startsWith(`/slides/m05/${aulaId}-`),
      `${slide.titulo} precisa apontar para um infografico do M05`,
    )
