...
const pilotLessonIds = ['aula-13-1', 'aula-13-2', 'aula-13-3', 'aula-13-4']
...
for (const aulaId of pilotLessonIds) {
  ...
  assert.equal(slides.length, 5, `${aulaId} deve ter 5 slides no piloto M13`)
  ...
  slides.forEach((slide, index) => {
    ...
    assert.ok(
      slide.imagem_url?.startsWith(`/slides/m13/${aulaId}-`),
      `${slide.titulo} precisa apontar para um infografico do M13`,
    )
