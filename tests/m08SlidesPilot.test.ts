...
const pilotLessonIds = ['aula-8-1', 'aula-8-2', 'aula-8-3', 'aula-8-4']
...
for (const aulaId of pilotLessonIds) {
  ...
  assert.equal(slides.length, 5, `${aulaId} deve ter 5 slides no piloto M08`)
  ...
  slides.forEach((slide, index) => {
    ...
    assert.ok(
      slide.imagem_url?.startsWith(`/slides/m08/${aulaId}-`),
      `${slide.titulo} precisa apontar para um infografico do M08`,
    )
