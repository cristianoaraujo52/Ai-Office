...
const pilotLessonIds = ['aula-9-1', 'aula-9-2', 'aula-9-3']
...
for (const aulaId of pilotLessonIds) {
  ...
  assert.equal(slides.length, 5, `${aulaId} deve ter 5 slides no piloto M09`)
  ...
  slides.forEach((slide, index) => {
    ...
    assert.ok(
      slide.imagem_url?.startsWith(`/slides/m09/${aulaId}-`),
      `${slide.titulo} precisa apontar para um infografico do M09`,
    )
