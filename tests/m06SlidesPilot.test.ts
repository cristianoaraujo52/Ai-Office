...
const pilotLessonIds = ['aula-6-1', 'aula-6-2', 'aula-6-3', 'aula-6-4']
...
for (const aulaId of pilotLessonIds) {
  ...
  assert.equal(slides.length, 5, `${aulaId} deve ter 5 slides no piloto M06`)
  ...
  slides.forEach((slide, index) => {
    ...
    assert.ok(
      slide.imagem_url?.startsWith(`/slides/m06/${aulaId}-`),
      `${slide.titulo} precisa apontar para um infografico do M06`,
    )
