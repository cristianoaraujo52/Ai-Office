...
const pilotLessonIds = ['aula-14-1', 'aula-14-2', 'aula-14-3']
...
for (const aulaId of pilotLessonIds) {
  ...
  assert.equal(slides.length, 5, `${aulaId} deve ter 5 slides no piloto M14`)
  ...
  slides.forEach((slide, index) => {
    ...
    assert.ok(
      slide.imagem_url?.startsWith(`/slides/m14/${aulaId}-`),
      `${slide.titulo} precisa apontar para um infografico do M14`,
    )
