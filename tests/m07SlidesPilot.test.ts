...
const pilotLessonIds = ['aula-7-1', 'aula-7-2', 'aula-7-3', 'aula-7-4']
...
for (const aulaId of pilotLessonIds) {
  ...
  assert.equal(slides.length, 5, `${aulaId} deve ter 5 slides no piloto M07`)
  ...
  slides.forEach((slide, index) => {
    ...
    assert.ok(
      slide.imagem_url?.startsWith(`/slides/m07/${aulaId}-`),
      `${slide.titulo} precisa apontar para um infografico do M07`,
    )
